...
|---|---|---|
| Transcripts generated by a frontier model via `--generate` | Transcripts from the same `Agent` class driven by the scripted `FunctionModel` (`pakka/sim/transcripts/naive/`) | No model key in the build environment. `modal run pakka/agent.py --generate` regenerates from any provider; the layer's decisions don't change because the anomalies are in the environment |
...
| Logfire trace screenshot and the checked-vs-held dashboard in the video | Placeholders in `video/assets/` | No `LOGFIRE_TOKEN` in the build environment; the spans are emitted |
