...
| Rule derivation from a pre-applied email edit, the rule card already open on arrival (§3, 1:15 block) | The person edits Ashcombe's email on the board; `POST /edit/preview` proposes the rule from that edit and the same popup asks "Make this a rule?"; accepted with the job's decisions | The pre-applied edit claimed a correction the person never made; the owner asked for the edit to be theirs |
