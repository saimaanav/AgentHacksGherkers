...
| 0:06 | Review: four vendor cards, one flagged — *the pay-to account on Halden's invoice isn't the account on file, and the agent read both*; Ashcombe's email still has the bank details in it | **Discard**, **Edit** the email and take the bank details out, **Yes always** on the rule the layer proposes from that edit, **Approve** | *"This one's an approved invoice with a new bank account on it — the layer saw the account on file in the same run. He discards it, and the entry and email that depended on it go with it. He pulls bank details out of an email, and that becomes a rule. Approve."* |
...
| `POST /edit/preview` | `EditRequest {run, write_id, args}` | `EditPreview {run, write_id, edited_args, proposed_rules[]}` | An edit the person has typed but not sent: the args as the tool's model reads them (422 with the fields it refused) and the rule the layer would propose from what was taken out. Nothing is saved |
...
- **Learning.** After Fridays 1–6 the ladder proposes release for the email tool first; the person's edit of Friday 1's email derives exactly one `Rule` (previewed with nothing saved, then accepted with the same decisions); that rule holds a matching send on the next call; `build_envelope` on a model with fields named `a`, `b`, `c` (an `int`, a low-cardinality `str`, an email-like `str`) produces a range, a set and a domain set; autopilot passes never widen the envelope.
