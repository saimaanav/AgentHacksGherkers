...
| Logfire dashboard screenshots in the video | `video/record.py` shows a captioned placeholder frame until `video/assets/logfire-trace.png` and `logfire-chart.png` exist (that directory is not in the repo yet) | The spans are emitted; the account is the missing piece |
