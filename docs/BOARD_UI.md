...
- **Edit** opens the args as a form (string, number, object fields from the tool's `args_schema` in `/scenario` or the connector's tools). **Keep these changes** posts `POST /edit/preview {run, write_id, args}`: a 422 lists the fields the tool's model refused, shown inline with the form kept open; a 200 carries the args as the model read them and, when the text taken out matches a pattern the layer knows, the rule it proposes — which opens in the same popup as "Make this a rule?". The decisions still go once per run; `POST /decide` derives the same rule from the edit and accepts or declines it with `accept_rules` / `reject_rules`.
...
A rule the layer proposes from the person's edit is a card of its own in *Needs your approval* ("Make this a rule?") until the decisions are sent; once accepted it sits in *Complete* as "Rule on" with a green edge, and a declined one leaves the board (it stays in the Rules sheet's history). Promotions (`run.proposed_promotions[]`) are accepted with the decisions. Nothing is edited for the person: Friday 1 opens with the bank details still in Ashcombe's email, and the proposal appears when the person takes them out. The rule card's popup is built so the person sees in a second what needs their eye and why, then says the rule in their own words:
...
2. Discard the flagged one (cascade shows 2 more will fall). Open Ashcombe's card, **Edit** the email, delete the sentence with the bank details, **Keep these changes**: the popup becomes "Make this a rule?" with the removed text first. **Yes, always**. Send decisions. Cards move; the rule card turns green in *Complete*; the rail fills.
