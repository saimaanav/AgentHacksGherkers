...
Proposed rules (`run.proposed_rules[]`) and promotions (`run.proposed_promotions[]`) appear as a small prompt above the columns after a run: "Make this a rule? hold `send_remittance_email` when `body` contains a sort code — [Yes, always] [Not now]". Yes puts the id into `accept_rules`.
