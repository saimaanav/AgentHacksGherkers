...
1. **What you took out, first.** An amber block: *You took **a sort code** out of the **body** of this **send remittance email***; under it the exact text the edit removed, with the match highlighted; then why it matters (nothing flagged it; the agent wrote it and it would have gone out as written; only the edit caught it). The whole field, before and after, is one click away, collapsed.
